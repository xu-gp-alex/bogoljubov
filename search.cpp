 ...
 */
std::vector<Move> extract_moves(const Board &board, bool k_castle, bool q_castle, i32 enemy_m2s, Side side) {
...
i32 mini(const Board &board, i32 depth);

i32 maxi(const Board &board, i32 depth) {
    if (depth == 0) return evaluate(board);
    i32 max = n_infty;
    std::vector<Move> all_moves;
    for (Move cand : all_moves) {
        i32 score = mini(board, depth - 1);
        if (score > max) {
            max = score;
        }
    }
